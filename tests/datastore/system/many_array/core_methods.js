...
module("hub.ManyArray core methods", {
  setup: function() {
...
test("changing storeIds length should change length of rec array also", function() {
  ...
  storeIds.pushObject(hub.Store.generateStoreKey()); // change length
...
test("reading a record not loaded in store should trigger retrieveRecord", function() {
  ...
  MyApp.store.removeDataHash(storeKey, hub.Record.EMPTY);
  MyApp.store.retrieveRecord = function() { callCount++; };
  
  var rec = recs.objectAt(0);
  equals(MyApp.store.readStatus(rec), hub.Record.EMPTY, 'precond - storeKey must not be loaded');
