...
var TestRecord = hub.Record.extend();
var TestRecord2 = hub.Record.extend();
...
function performBasicTests(methodName, loc) {
  ...
  module("hub.Query.%@()".fmt(methodName), {
    setup: function() {
    ...
    teardown: function() {
      GLOBAL.TestRecord = GLOBAL.TestRecord2 = null; // cleanup
    }
  });

  function invokeWith() {
    return hub.Query[methodName].apply(hub.Query, arguments);
  }
  ...
  test("query with multiple recordtypes", function() {
    ...
    set = hub.Set.create().add(TestRecord).add(TestRecord2);
    q4  = invokeWith(set);
  ...
  test("query with no record type and with conditions", function() {
    ...
    queryEquals(q1, loc, hub.Record, 'foobar', 'first query');
  ...
  test("passing query object", function() {
    ...
    var local = hub.Query.local(TestRecord),
        remote = hub.Query.remote(TestRecord),
        q;
        
    q = invokeWith(local);
    if (loc === hub.Query.LOCAL) {
  ...
  test("no options (matches everything)", function() {
    ...
    q1 = invokeWith(); 
    queryEquals(q1, loc, hub.Record, null, 'first query - matches everything');
...
performBasicTests('local', hub.Query.LOCAL);
performBasicTests('remote', hub.Query.REMOTE);
