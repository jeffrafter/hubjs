...
module("hub.Store#commitChangesFromNestedStore", {
  setup: function() {
...
test("ignores changed data hashes not passed in changes set", function() {
  ...
  // perform action
  equals(store.commitChangesFromNestedStore(child, hub.Set.create(), false), store, 'should return receiver');
...
function createConflict(force) {
  ...
  try {
    ...
    equals(e, hub.Store.CHAIN_CONFLICT_ERROR, 'should throw CHAIN_CONFLICT_ERROR');
    errorCount++;
  }  
