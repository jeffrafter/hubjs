...
module("hub.RecordArray core methods", {
  setup: function() {
    ...
    // get record array.
    storeKeys = [storeKey];
    recs = hub.RecordArray.create({ store: store, storeKeys: storeKeys });
  }
});

test("initial status", function() {
  equals(recs.get('status'), hub.Record.EMPTY, 'status should be hub.Record.EMPTY');
});
...
test("changing storeKeys length should change length of rec array also", function() {
  ...
  storeKeys.pushObject(hub.Store.generateStoreKey()); // change length
...
test("reading a record not loaded in store should trigger retrieveRecord", function() {
  ...
  // loaded.
  store.removeDataHash(storeKey, hub.Record.EMPTY);
  store.retrieveRecord = function() { callCount++; };
  
  equals(store.peekStatus(storeKeys.objectAt(0)), hub.Record.EMPTY, 'precond - storeKey must not be loaded');
...
test("changing the underlying storeKeys should notify observers of records", function() {
  ...
  storeKeys.pushObject(hub.Store.generateStoreKey());
