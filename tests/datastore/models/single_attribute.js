...
module("hub.RecordAttribute core methods", {
  setup: function() {
    ...
    MyApp.Foo = hub.Record.extend({
      ...
      bar: hub.Record.toOne('MyApp.Bar', { inverse: 'foo' }),
      
      barKeyed: hub.Record.toOne('MyApp.Bar', { key: 'barId' })
...
test("clearing a toOne relationship with an inverse - foo isMaster", function() {
  ...
  equals(rec.get('status'), hub.Record.READY_CLEAN, 'precond - foo1.status should be READY_CLEAN');
  equals(bar.get('status'), hub.Record.READY_CLEAN, 'precond - bar1.status should be READY_CLEAN');
  ...
  equals(rec.get('status'), hub.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(bar.get('status'), hub.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
...
test("modifying a toOne relationship with an inverse from null", function() {
  ...
  equals(rec.get('status'), hub.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(rec2.get('status'), hub.Record.READY_DIRTY, 'foo1.status should be READY_DIRTY');
  equals(bar.get('status'), hub.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
...
test("modifying a toOne relationship with an inverse from other", function() {
  ...
  equals(bar2.get('status'), hub.Record.READY_CLEAN, 'bar1.status should be READY_CLEAN');
