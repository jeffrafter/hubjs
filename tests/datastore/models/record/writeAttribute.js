...
test("Writing to an attribute in a child store sets correct status", function() {
  var childStore = store.createEditingContext(),
      rec = childStore.find(Foo, foo.get('id'));
  
  equals(rec.get('status'), hub.Record.READY_CLEAN, 'precon - status should be READY_CLEAN');
  rec.writeAttribute('foo', 'newValue');
  //chainedRecord.set('foo', 'newValue');
  equals(rec.get('status'), hub.Record.READY_DIRTY, 'status should be READY_DIRTY');
});
