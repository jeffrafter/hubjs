...
test("fetching the object at index", function() {
  var ary = hub.SparseArray.create(10);
...
test("Update the sparse array using provideObjectAtIndex", function() {
  var ary = hub.SparseArray.create(2);
  var obj = "not";
  ary.provideObjectAtIndex(0, obj);
  equals(obj, ary._hub_sa_content[0],"Content at 0th index");
  obj = "now";
  ary.provideObjectAtIndex(1, obj);
  equals(obj, ary._hub_sa_content[1],"Content at 1st index");
});

test("objectAt() should get the object at the specified index",function() {
  var spArray = hub.SparseArray.create(4) ;
  var arr = [hub.Object.create({ dummy: true }),"Sproutcore",2,true];
  spArray = arr;
...
test("element to be added is at idx > length of array ", function() {
  var ary = hub.SparseArray.array(5) ;
...
test("modifying a range should not require the rest of the array to refetch", function() {
  ...
  var ary = hub.SparseArray.create({
    delegate: del
  });
...
test("Check that requestIndex works with a rangeWindowSize larger than 1", function() {
  var ary = hub.SparseArray.array(10) ;
  var didRequestRange=false;
  
  var DummyDelegate = hub.Object.extend({
...
test("definedIndexes", function() {
  ...
  var expected = hub.IndexSet.create().add(5);
  same(ary.definedIndexes(), expected, 'definedIndexes() should return all defined indexes');
  
  same(ary.definedIndexes(hub.IndexSet.create().add(2, 10)), expected, 'definedIndexes([2..11]) should return indexes within');
  
  same(ary.definedIndexes(hub.IndexSet.create().add(2)), hub.IndexSet.EMPTY, 'definedIndexes([2]) should return empty set (since does not overlap with defined index)');
  
});

// ..........................................................
// TEST hub.ARRAY COMPLIANCE
// 
...
Spin.ArraySuite.generate("hub.SparseArray", {
  newObject: function(amt) {
    ...
    return hub.SparseArray.create({ delegate: del });
  }
});
