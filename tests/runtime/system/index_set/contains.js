...
test("handle index in set", function() {
  equals(set.contains(1001), true, hub.fmt('index 1001 should be in set %@', set));
  equals(set.contains(1009), true, hub.fmt('index 1009 should be in set %@', set));
  equals(set.contains(2000), true, hub.fmt('index 2000 should be in set %@', set));
});
...
test("handle set inside set", function() {
  ...
  equals(set.contains(test), true, hub.fmt('%@ should be in %@', test, set));
});

test("handle range outside of set", function() {
  var test = hub.IndexSet.create().add(100,4).add(105,2);
  equals(set.contains(test), false, hub.fmt('%@ should be in %@', test, set));
});
