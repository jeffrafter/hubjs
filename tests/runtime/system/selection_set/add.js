...
test("Adding indexes for single source", function() {
  set.add(array, 4, 3);
  validate(set, [hub.IndexSet.create(4,3)], array);

  set.add(array, 1);
  validate(set, [hub.IndexSet.create(1).add(4,3)], array);
});

test("Adding multiple sources", function() {
  var expected = hub.IndexSet.create(4,3);
  var expected2 = hub.IndexSet.create(1);
...
test("Adding another SelectionSet", function() {
  var expected = hub.IndexSet.create(4,3);
  var expected2 = hub.IndexSet.create(1,5);
  expected.source = array;
  expected2.source = array2;
  ...
  var set2 = hub.SelectionSet.create().add(array2, 1, 5);
