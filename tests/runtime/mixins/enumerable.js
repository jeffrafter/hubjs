...
var DummyEnumerable = hub.Object.extend( hub.Enumerable, {
...
test("should map to values while passing proper params", function() {
  ...
  for (var idx2=0, len2=ary2.length; idx2<len2; idx2++) {
    ...
    for(idx=0;idx<len;idx++) {
      ...
      equals(hub.guidFor(this), hub.guidFor(targets[idx]), "this") ;
...
test("should trigger observer of reduced prop when array changes once property retrieved once", function() {
  ...
  for (var idx2=0, len2=ary2.length; idx2<len2; idx2++) {
    ...
    //hub.NotificationQueue.flush() ; // force observers to trigger
...
/*
  ...
  Arrays (where the hub.Object.init method is not run).  

  The fix for this problem was to add an initObservable() method to 
  hub.Observable that will configure bindings and dependent keys.  This
  method is called from hub.Object.init() and it is called in
  hub.Observable._notifyPropertyChanges if it has not been called already.

  hub.Enumerable was in turn modified to register reducers as dependent 
